...
const SLIDES_WORKFLOW_TEXT = [
  ...
  "## 多演示文稿页面",
  "",
  "- 「页」有两层含义:**项目页面** = 一个独立演示文稿文件(manifest.pages 条目),",
  "  **演示页** = 该文件 deck 内的一个 `.slide`;",
  "- 每个项目页面 HTML 自带一个 `.deck`,内含该演示文稿的全部 `.slide`;",
  "  通常一个 PPT 项目就是一个演示文稿(单个入口文件);",
  "- 「+」新建会得到一个新的空白演示文稿(单页 slide 起始),注册到",
  "  manifest.pages,manifest.entry 恒为当前活动演示文稿;",
  "- Studio 画布一次只渲染当前活动页(当前演示文稿),翻页导航在该页的 deck 内;",
  "- 导出作用于**当前页面**(当前演示文稿)的全部幻灯片;页面必须含 `.slide`",
  "  结构才会显示导出菜单。",
  "",
  "## 设计令牌系统",
  "",
