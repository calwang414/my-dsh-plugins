 ...
 * scope 装载一次。三个职责:
 *
 * 1. 向 system-prompt 注册「设计工作流」段:通用部分压到最少,差异化部分
 *    按「当前激活视图模式」动态注入(host 端记录用户正在查看 Design 视图
 *    还是 PPT 视图,每次组装提示词时读取并按模式分支);
 * 2. 网页/PPT 各自的工作流细节(令牌系统、画布契约、完整步骤)在对应的
 *    技能文档 `design-workflow` / `ppt-workflow` 中,提示词只保留关键约束
 *    与自查清单,避免与技能文档重复;
...
/** 双模式共用、必须每次常驻的最小不变量;细节全部交给对应技能文档。 */
const COMMON_WORKFLOW_TEXT = [
  ...
  '',
  '开始任务前先加载技能目录中的对应工作流技能(`design-workflow` 网页设计 /',
  '`ppt-workflow` 演示文稿),按其中的项目结构、令牌系统与工作流契约执行。',
  '',
  '核心不变量:只修改 {{cwd}}/design/ 内的文件;样式一律通过 design-tokens.css',
  '的 --ipw-* 变量控制;保存前先读当前文件,避免覆盖其他设计模式会话的最新修改。',
].join('\n')

/** 网页设计项目的差异化约束与自查清单(Design 视图,manifest.category 非 slides)。 */
const DESIGN_PROJECT_TEXT = [
  '',
  '## 当前项目:网页设计(Design 视图)',
  '你正在处理 {{cwd}}/design/ 下的网页设计项目(manifest.category 非 slides),',
  '按 `design-workflow` 技能执行。',
  '',
  '网页项目专属约束:',
  '- 页面为普通网页 HTML,响应式适配窄视口;',
  '- 多页面:每页一个独立 HTML 文件,注册到 manifest.pages,entry 恒为活动页;',
  '- 设计规范与布局分离:引入模板布局不改 design-tokens.css;',
  '  新项目先向用户推荐并写入设计规范,再开始设计。',
  '',
  '网页项目完成后自查(review):',
...
/** 演示文稿(PPT)项目的差异化约束与自查清单(PPT 视图,manifest.category 为 slides)。 */
...
const SELF_DETECT_TEXT = [
  ...
  '{{cwd}}/design/ppt/manifest.json 的 category 字段,判断目标项目后按对应技能',
  '(`design-workflow` 或 `ppt-workflow`)与其自查清单执行。',
].join('\n')
...
export function buildWorkflowText(mode) {
  ...
  if (mode === 'slides') {
    parts.push(SLIDES_PROJECT_TEXT)
  } else if (mode === 'design') {
    parts.push(DESIGN_PROJECT_TEXT)
  } else {
