...
// ---- 固定目录写入（File System Access API）----
// 首次导出让用户选择一个输出目录(建议 design/output),句柄存入 IndexedDB,
// 之后每次导出直接写入该固定目录,不再触发浏览器下载。

const DIR_DB_NAME = "dsh-design-source-output";
const DIR_DB_STORE = "dirs";
const DIR_DB_KEY = "output";

function openDirDb() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(DIR_DB_NAME, 1);
    request.onupgradeneeded = () => request.result.createObjectStore(DIR_DB_STORE);
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error);
  });
}

async function savedOutputDir() {
  const db = await openDirDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(DIR_DB_STORE, "readonly");
    const request = tx.objectStore(DIR_DB_STORE).get(DIR_DB_KEY);
    request.onsuccess = () => resolve(request.result || null);
    request.onerror = () => reject(request.error);
  });
}

async function persistOutputDir(handle) {
  const db = await openDirDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(DIR_DB_STORE, "readwrite");
    tx.objectStore(DIR_DB_STORE).put(handle, DIR_DB_KEY);
...
async function ensureOutputDir() {
  if (typeof window.showDirectoryPicker !== "function") {
    throw new Error("This browser cannot save into a folder. Use a recent Chrome or Edge.");
  }
  const saved = await savedOutputDir();
  if (saved) {
    const permission = await saved.queryPermission({ mode: "readwrite" });
    if (permission === "granted") return saved;
  }
  const picked = await window.showDirectoryPicker({ mode: "readwrite" });
  if (!picked) throw new Error("No output folder selected.");
  await persistOutputDir(picked);
  return picked;
}

async function writeToOutput(blob, filename) {
...
async function exportSketch(ctx) {
  ...
  return writeToOutput(blob, ctx.title + ".sketch");
}
...
async function exportPsd(ctx) {
  ...
  return writeToOutput(new Blob([buffer], { type: "application/octet-stream" }), ctx.title + ".psd");
}
...
export async function exportDesignSource(kind) {
  ...
  try {
    if (kind === "sketch") {
      return await exportSketch(ctx);
    } else if (kind === "psd") {
      return await exportPsd(ctx);
    } else {
