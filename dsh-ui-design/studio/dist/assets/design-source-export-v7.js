...
// 把生成的源文件上传到 Host,写入项目的 design/[projectId]/output/ 固定目录
// (不是浏览器下载)。api: { base, token(), workspaceId, sessionId } 由主 bundle 注入。
async function uploadSource(blob, filename, api) {
  ...
  const res = await fetch(api.base + "/design-source", {
    method: "POST",
    headers: { "content-type": "application/json", "x-dsh-ui-design-token": api.token() },
    body: JSON.stringify({ workspaceId: api.workspaceId, sessionId: api.sessionId, filename, data }),
  });
  const json = await res.json().catch(() => null);
  if (!res.ok || !json) throw new Error((json && json.message) || "Failed to save the design source.");
  return json;
}
...
async function exportSketch(ctx) {
  ...
  return uploadSource(blob, ctx.title + ".sketch", ctx.api);
}
...
async function exportPsd(ctx) {
  ...
  return uploadSource(new Blob([buffer], { type: "application/octet-stream" }), ctx.title + ".psd", ctx.api);
}

// ---- 入口 ----

export async function exportDesignSource(kind, api) {
